...
public class DualContouringGenerator : MonoBehaviour
{
    ...
    public struct GLM : Octree.IGenerator
    {
        ...
        public float Density_Func(float3 position)
        {
    ...
    IEnumerator GenMesh(Mesh mesh)
    {
        thresholdIndex = (thresholdIndex + 1) % MAX_THRESHOLDS;
        while(!destroyCancellationToken.IsCancellationRequested)
        {
            var nodes = new NativeList<Octree.Node>(8 * (((int)math.pow(octreeSize,3) * 8) - 1) / (8 - 1),Allocator.Persistent);
            ...
            var genMeshJob = new Octree.GenMeshJob() {
                nodes = nodes,
    ...
    void Start()
    {
        ...
        StartCoroutine(GenMesh(filter.mesh));
	}
